...
This is a Docker image that creates, compresses, and saves a MariaDB database dump to a targeted directory in `.sql.gz` format. The filename will be a timestamp based on the ISO-8601 standard (yyyy-mm-ddThh:mm:ss).

The dump file (which will be named `dump.sql.gz`) is placed into /data/dumps which is a volume mount point that should be mounted locally or in another container.

To build the image, simply run, e.g.

```
docker build . -t mariadb-database-backup
```
