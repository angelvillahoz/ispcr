...
While not strictly required, using Docker is highly recommended to compile and run the service. Use the `Dockerfile` to build the image (e.g. `docker build . -t blatserver`, then run it, making sure to map the port for web access and the volume for the assets (more below). An example command could look like `docker run --rm -dp 80:8080 -v $PWD/assets:/assets blatserver`.
...
The same one for the `agam4.2bit` genome database file of the Anopheles gambiae species.
