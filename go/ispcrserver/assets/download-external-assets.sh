...
# 
# dm1
wget -O - http://hgdownload.soe.ucsc.edu/goldenPath/dm1/bigZips/dm1.2bit > ./dm1.2bit
./twoBitToFa dm1.2bit dm1.fasta
echo "Filtering the chromosome names..."
awk '{ if ((NR>1)&&($0~/^>/)) { printf("\n%s", $0); } else if (NR==1) { printf("%s", $0); } else { printf("\t%s", $0); } }' dm1.fasta | grep -E -i -w "chr3R|chr3L|chr2R|chrX|chr2L|chrY|chr4" - | tr "\t" "\n" > skinned_dm1.fasta
rm dm1.fasta
echo "Building up dm1.2bit..."
./faToTwoBit skinned_dm1.fasta dm1.2bit
rm skinned_dm1.fasta
./twoBitInfo dm1.2bit stdout | sort -k2rn > dm1_chromosomes_information.txt
# dm2
wget -O - http://hgdownload.soe.ucsc.edu/goldenPath/dm2/bigZips/dm2.2bit > ./dm2.2bit
./twoBitToFa dm2.2bit dm2.fasta
echo "Filtering the chromosome names..."
awk '{ if ((NR>1)&&($0~/^>/)) { printf("\n%s", $0); } else if (NR==1) { printf("%s", $0); } else { printf("\t%s", $0); } }' dm2.fasta | grep -E -i -w "chr3R|chr3L|chr2R|chrX|chr2L|chrY|chr4" - | tr "\t" "\n" > skinned_dm2.fasta
rm dm2.fasta
echo "Building up dm2.2bit..."
./faToTwoBit skinned_dm2.fasta dm2.2bit
rm skinned_dm2.fasta
./twoBitInfo dm2.2bit stdout | sort -k2rn > dm2_chromosomes_information.txt
# dm3
wget -O - http://hgdownload.soe.ucsc.edu/goldenPath/dm3/bigZips/dm3.2bit > ./dm3.2bit
./twoBitToFa dm3.2bit dm3.fasta
echo "Filtering the chromosome names..."
awk '{ if ((NR>1)&&($0~/^>/)) { printf("\n%s", $0); } else if (NR==1) { printf("%s", $0); } else { printf("\t%s", $0); } }' dm3.fasta | grep -E -i -w "chr3R|chr3L|chr2R|chrX|chr2L|chrY|chr4" - | tr "\t" "\n" > skinned_dm3.fasta
rm dm3.fasta
echo "Building up dm3.2bit..."
./faToTwoBit skinned_dm3.fasta dm3.2bit
rm skinned_dm3.fasta
./twoBitInfo dm3.2bit stdout | sort -k2rn > dm3_chromosomes_information.txt
# dm6
